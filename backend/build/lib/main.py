...
# In-memory storage for user behavior samples (for demo purposes)
...
class PredictionResponse(BaseModel):
    ...
    action: Literal["ALLOW", "OTP_REQUIRED", "BLOCK"]
...
def normalize_features(features: list[float]) -> np.ndarray:
    ...
    bounds = [
        ...
        (1, 300),       # session_duration: 1-300 seconds
        (0, 50000),     # mouse_distance: 0-50000 pixels
...
def determine_risk_level(anomaly_score: float) -> tuple[Literal["LOW", "MEDIUM", "HIGH"], Literal["ALLOW", "OTP_REQUIRED", "BLOCK"]]:
...
async def predict(data: BehaviorData) -> PredictionResponse:
    ...
    For new users or users with few samples, we use conservative scoring.
    ...
    # If we still don't have a model, use conservative scoring (allow)
    if model is None:
    ...
    anomaly_score = compute_anomaly_score(model, features)
    risk_level, action = determine_risk_level(anomaly_score)
